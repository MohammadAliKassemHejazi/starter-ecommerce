    ...
    <div className={styles["image-container"]}>
      ...
      {productImages.map((image: ImageType | any, index) => (
        ...
        <div key={index} onClick={() => handleImageClick(index)}>
          <Image
              src={(isonline === true) ? (process.env.NEXT_PUBLIC_BASE_URL_Images + (image.url || image.imageUrl)) : (image.data_url ?? "")}
          ...
          >
            {productImages.map((image: ImageType | any, index) => (
              <SwiperSlide key={index} className={styles["swiper-slide"]}>
                <DynamicSizedImage url={(isonline === true) ? (process.env.NEXT_PUBLIC_BASE_URL_Images + (image.url || image.imageUrl)) :(image.data_url ?? "")} index={index} />
              </SwiperSlide>
            ))}
          </Swiper>
        </div>
