...
export const errorHandler = (error: Error, req: Request, res: Response, next: NextFunction) => {
  ...
  if (error instanceof CustomError) {
    return ResponseFormatter.error(res, error.message, error.statusCode, error.data ? [error.data] : undefined);
  }
  ...
  if (error.name === 'SequelizeUniqueConstraintError') {
    return ResponseFormatter.error(res, 'A record with this information already exists', 409);
  }
  ...
  return ResponseFormatter.error(res, process.env.NODE_ENV === 'production' ? 'Internal server error' : error.message, 500);
};
