...
async function seedDatabase() {
  try {
    ...
    const electronicsId = categories.find((category : any) => category.name === 'Electronics')?.id;
    ...
    const clothingId = categories.find((category:any) => category.name === 'Clothing')?.id;
    await db.Subcategory.bulkCreate([
    ...
    const booksId = categories.find((category:any) => category.name === 'Books')?.id;
    await db.Subcategory.bulkCreate([
