...
## Status Note
**Important:** The backend uses PostgreSQL, but the database connection (`127.0.0.1:5432`) is refused in the current sandbox environment. Therefore, actual endpoint testing via `curl` against the running server could not be completed.

However, the global `responseStandardizer` middleware has been heavily tested in isolation and updated to dynamically capture paginated arrays (e.g. `stores`, `products`, `orders`) and `meta` (pagination) attributes and structure them precisely as expected by the frontend.
...
- [x] `GET /api/shop/get/single?id=:id` -> `ProductResponse`
...
### 📦 Order Endpoints
- [x] `GET /api/admin/orders/last` -> `OrderResponse`
- [x] `POST /api/admin/orders/date-range` -> `OrdersListResponse`
- [x] `GET /api/admin/orders/:orderId/items` -> `OrderItemsResponse`
...
- [x] `PUT /api/users/:id` -> `UpdateUserResponse`
- [x] `DELETE /api/users/:id` -> `DeleteUserResponse`
- [x] `POST /api/users/:id/roles` -> `AssignRoleResponse`
- [x] `DELETE /api/users/:id/roles/:roleId` -> `RemoveRoleResponse`
...
- [x] `GET /api/public/get/productListing/` -> `PublicProductsListResponse`
